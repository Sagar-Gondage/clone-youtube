...
export const PlaylistAddModal = ({ open, onOpenChange, videoId }: PlaylistAddModalProps) => {
    ...
    const handleOpenChange = (newOpen: boolean) => {
        // clear the cache before closing the modal
        queryClient.removeQueries({
            predicate: (query) =>
                Array.isArray(query.queryKey[0]) &&
                query.queryKey[0][0] === "playlists" &&
                query.queryKey[0][1] === "getManyForVideo" &&
                (query.queryKey[1] as { input?: { videoId?: string } })?.input?.videoId === videoId
        })
    ...
    return (
        <ResponsiveModal title="Add to playlist" open={open} onOpenChange={handleOpenChange}>
            <div className="flex flex-col gap-2">
                ...
                {
                    !isLoading && <InfiniteScroll hasNextPage={hasNextPage} isFetchingNextPage={isFetchingNextPage} fetchNextPage={fetchNextPage} isManual={true} />
                }
            </div>
        </ResponsiveModal>
    )
