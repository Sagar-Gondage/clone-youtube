...
export const PlaylistThumbnail = ({ imageUrl, title, videoCount, className }: PlaylistThumbnailProps) => {
    ...
    return (
        <div className={cn("relative pt-3", className)}>
            ...
            {/* Vieo Counts */}
            <div className="absolute bottom-2 right-2 px-1 py-0.5 rounded bg-black/80 text-white text-xs font-medium flex items-center gap-x-2">
                <ListVideoIcon className="size-4" />
                {compactViews} video{videoCount !== 1 ? "s" : ""}
